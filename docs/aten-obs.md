...
4. **Browser Source** → `http://127.0.0.1:8000/overlay`, 1920×1080, Transform → Reset Transform, shutdown when not visible. Do not scale the source (that squashes type). Two-line captions in a `/control` box (default full width, 30% height). After overlay HTML changes, right-click → Refresh.
